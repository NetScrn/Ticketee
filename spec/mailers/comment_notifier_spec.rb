...
RSpec.describe CommentNotifier, type: :mailer do
  describe "created" do
    ...
    it "sends out an email notification about a now comment" do
      expect(email.to).to include ticket_owner.email
      title = "#{ticket.name} for #{project.name} has been updated."
      expect(email.to_s).to include title
      expect(email.to_s).to include "#{commenter.email} wrote:"
      expect(email.to_s).to include comment.text
    end
  end
end
